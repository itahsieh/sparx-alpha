...
/* Read molecular parameters from a LAMDA format data file */
{
	...
	for(i = 0; i < mp->nlev; i++) {
		...
		if(!status) { /* Get fifth column (quantum state) */
			if((sptr2 = strtok(NULL, "\t")))
				mp->lev[i]->F1 = Mem_STRDUP(MemStr_Stripwhite(sptr2));
			else
				status = Err_SETSTRING("In file `%s': Error reading level quantum state (F1) at row %d", fname, i + 1);
		}
		
		if(!status) { /* Get sixth column (quantum state) */
			if((sptr2 = strtok(NULL, "\0")))
				mp->lev[i]->F = Mem_STRDUP(MemStr_Stripwhite(sptr2));
			else
				status = Err_SETSTRING("In file `%s': Error reading level quantum state (F) at row %d", fname, i + 1);
		}
	}
	...
	/* load line info from nline lines after line 19 + nlev */
	if(!status) {
		if(!(sptr = get_nline(fp, 19 + mp->nlev)))
			status = Err_SETSTRING("In file `%s': Radiative transitions not found", fname);
