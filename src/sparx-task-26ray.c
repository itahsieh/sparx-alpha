...
static struct glb {
	...
	int stage, fully_random, lte, trace;
...
static void CalcJbar(size_t tid, SpPhys *pp, const double *ds0, const double *vfac0,
	const double *intensity, const double *tau);
...
int SpTask_Amc(void)
{
	...
	if(!sts && glb.model.parms.mol != NULL) {
		PyWrErr_SetString(PyExc_Exception, "Model must not already contain pops");
		sts = 1;
	}
	/* Load molecule */
	if(!sts) sts = SpPy_GetInput_molec("molec", &glb.model.parms.mol);
...
static int CalcExc(void)
{
	...
	for(glb.stage = 0; !sts && (glb.stage < STAGE_N); glb.stage++) {
		...
		for(iter = 0; !sts && (glb.nconv < (glb.stage? (size_t)(0.998*(double)glb.nzone+0.5):glb.nzone) ); iter++) {
			...
			if(glb.trace && Sp_MPIRANK == 0) {
				if(!sts) {
					stmp = Mem_Sprintf("%s.stage%d-%05d", glb.outf->name, glb.stage, iter);
			...
			if(record){
				if(glb.stage==1){
					...
					fprintf(fp,"POINTS %zu float\n",zone->nchildren);
					//for(izone = 0; izone < glb.nzone; izone++){					
					for(izone=0; izone < zone->nchildren; izone++ ){
						//zp = glb.zones[izone];
						zp = zone->children[izone];
	...
	if(1){
		sprintf(filename,"SP__final.vtk",iter);
		fp=fopen(filename,"w");
		fprintf(fp,"# vtk DataFile Version 3.0\n");
		fprintf(fp,"MODEL\n");
		fprintf(fp,"ASCII\n");
...
static void *CalcExcThread(void *tid_p)
{
	...
	double *ds0, *vfac0, *intensity, *tau, *hist, diff, vel,temp;
	double *popsold;
	...
	popsold = Mem_CALLOC(NLEV, popsold);
	...
	for(izone = 0; izone < glb.nzone; izone++) {
		...
		for(ihist = 0; ihist < (glb.stage? NHIST:1); ihist++) {
			if(!glb.fully_random)
		...
		if (glb.stage){
			...
			for(i = 0; i < NLEV; i++) {
					...
					temp = fabs( pp->pops[tid][i] - popsold[i] ) / pp->pops[tid][i];
	...
	if(Sp_MPISIZE > 1) {
		...
		if(Sp_MPIRANK != 0) {
			...
			if(zone_rank == Sp_MPIRANK) {
				...
				//MPI_Send(&pp->nray, 1, MPI_INT, 0, Sp_MPITAG, MPI_COMM_WORLD);
				//MPI_Send(&pp->diff, 1, MPI_DOUBLE, 0, Sp_MPITAG, MPI_COMM_WORLD);
				//MPI_Send(&pp->converged, 1, MPI_INT, 0, Sp_MPITAG, MPI_COMM_WORLD);
			}
		}
		else {
			...
			if(zone_rank != 0) {
				...
				//MPI_Recv(&pp->nray, 1, MPI_INT, (int)zone_rank, Sp_MPITAG, MPI_COMM_WORLD, &mpi_status);
				//MPI_Recv(&pp->diff, 1, MPI_DOUBLE, (int)zone_rank, Sp_MPITAG, MPI_COMM_WORLD, &mpi_status);
				//MPI_Recv(&pp->converged, 1, MPI_INT, (int)zone_rank, Sp_MPITAG, MPI_COMM_WORLD, &mpi_status);
			}
		}
		...
		//MPI_Bcast(&pp->nray, 1, MPI_INT, 0, MPI_COMM_WORLD);
		//MPI_Bcast(&pp->diff, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		//MPI_Bcast(&pp->converged, 1, MPI_INT, 0, MPI_COMM_WORLD);
	}
	#endif
 ...
 */
{
	...
	for(i = 0; i < pp->nray; i++) {
		...
		vel = (PRAND() - 0.5) * 4.3 * pp->width + GeVec3_DotProd(&v_gas, &ray.d);
 ...
 * along the ray from the ray origin to the edge of the cloud */
{
	...
	Zone *zp = zone;
	SpPhys *pp, *zone_pp;
	double t, ds = 0, vfac = 0, j_nu, k_nu, S_nu, *dtau = 0;
	size_t plane;
	...
	*vfac0 = 0;
	...
	while(zp) {
		...
		if(pp->non_empty_leaf) {
			...
			vfac = pp->has_tracer ? SpPhys_GetVfac(ray, t, vel, zp, 0) : 0.0;
			...
			for(i = 0; i < NRAD; i++) {
				...
				S_nu = fabs(k_nu) > 0.0 ? j_nu / k_nu / glb.I_norm[i] : 0.0;
 ...
 * populations, and save results in pp */
{
	...
	for(iter = 0; iter < glb.maxi; iter++) {
		for(ihist = 0; ihist < NHIST; ihist++) {
			...
			CalcJbar(tid, pp, ds0, vfac0, intensity, tau);
			...
			for(i = 0; i < NRAD; i++) {
				...
				RMAT(up, up) -= (RAD(i)->A_ul + pp->J_bar[i] * RAD(i)->B_ul);
				RMAT(lo, lo) -= (pp->J_bar[i] * RAD(i)->B_lu);

				/* Off-diagonal terms are transitions `into' row state */
				RMAT(up, lo) += (pp->J_bar[i] * RAD(i)->B_lu);
				RMAT(lo, up) += (RAD(i)->A_ul + pp->J_bar[i] * RAD(i)->B_ul);
			}
