...
def writegrid(direc,lev,naxes,Raxis,Taxis,Paxis,nc):
	...
	if (writevtk):
        	...
        	print >>fvtk1,'DIMENSIONS %(0)d %(1)d %(2)d'%{'0':nr+1,'1':nt+1,'2':np+1}
		...
		fvtk2=open(vtkfileb, mode = "w")
        	...
        	print >>fvtk2,'DIMENSIONS %(0)d %(1)d %(2)d'%{'0':nr,'1':nt,'2':np}
        	print >>fvtk2,'POINTS %(0)d float'%{'0':nr*nt*np}
		for j in range(nt):
			for i in range(nr):
				print >>fvtk2,'%(0)e %(1)d %(2)e'%{'0':0.5*(Rp[i]+Rp[i+1])*sin(0.5*(thetap[j]+thetap[j+1])),'1':0,'2':Rp[i]*cos(thetap[j])}
